...
import io.fintechlabs.testframework.condition.client.CreateAuthorizationEndpointRequest;
...
import io.fintechlabs.testframework.condition.client.SetAuthorizationEndpointClientIdFromClientInformation;
 ...
 */
@PublishTestModule(
	testName = "openid-op-redirect-uri-missing",
	displayName = "OpenID OP-Redirect_Uri-Missing",
	profile = "OIDFBasic",
	configurationFields = {
		"server.discoveryUrl",
		"client.client_id",
	...
	summary = "Reject request without redirect_uri when multiple registered"
)
public class OpenId_OP_Redirect_Uri_Missing extends OpenIdBaseModule {

	public static Logger logger = LoggerFactory.getLogger(OpenId_OP_Redirect_Uri_Missing.class);
	...
	protected void configureDynamicClientRegistration(JsonObject config) {
		...
		redirectUris.add(redirectUri2);
		config.add("redirect_uris", redirectUris);
	}
	...
	protected void configureAuthorizationRequestParameters() {
		...
		callAndStopOnFailure(SetAuthorizationEndpointClientIdFromClientInformation.class);
	}
	...
	public void configure(JsonObject config, String baseUrl) {
		...
		env.putString("redirect_uri_suffix", "_1");
		callAndStopOnFailure(CreateRedirectUri.class);
		// this is inserted by the create call above, expose it to the test environment for publication
		exposeEnvString("redirect_uri");

		redirectUri1 = env.getString("redirect_uri");

		env.putString("redirect_uri_suffix", "_2");
		callAndStopOnFailure(CreateRedirectUri.class);
		// this is inserted by the create call above, expose it to the test environment for publication
		exposeEnvString("redirect_uri");

		redirectUri2 = env.getString("redirect_uri");
...
//		callAndContinueOnFailure(DisallowInsecureCipher.class, ConditionResult.FAILURE, "FAPI-RW-8.5-2");
		...
		callAndStopOnFailure(CreateAuthorizationEndpointRequest.class);
